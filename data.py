...
class Data():
    ...
    def total_lost(self):
        ...
        for unit in list(Data.AGENTUNITS):
            for individual in Data.AGENTUNITS[unit]:
    ...
    def enemy_total_lost(self):
        """Determines the total lost value of perished Enemy Units"""

        enemy_lost_minerals = 0
        enemy_lost_gas = 0

        for unit in list(Data.ENEMYUNITS):
            for individual in Data.ENEMYUNITS[unit]:
                if not individual.is_alive:
                    enemy_lost_minerals += individual.unit_type.mineral_price
                    enemy_lost_gas += individual.unit_type.gas_price

        return [enemy_lost_minerals, enemy_lost_gas]
